...
if ($tags) {
    ...
    foreach ($tags as $tag) {
        $tags_list .= '<a href="' . esc_url(get_tag_link($tag->term_id)) . '" class="custom-tag">' . $svg_icon . esc_html($tag->name) . '</a>, ';
    }
    // Eliminar la última coma y espacio
    $tags_list = rtrim($tags_list, ', ');

    echo '<div class="tags-list">Etiquetas: ' . $tags_list . '</div>';
}
